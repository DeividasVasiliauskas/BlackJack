...
def dealer(i):
    print(f"Dealer reveals: {real_cards(d_cards)[0]} {real_cards(d_cards)[1]}")
    card_sum = sum_cards(i)
    while sum_cards(i) < 16:
        print("Sum bellow 16 HITTING")
        hit("dealer")
        print(game_dict["dealer"])
        card_sum = sum_cards(i)
    if sum_cards(i) > 21:
        card_sum = 0
    return card_sum
...
def play_again():
    finished = input("Do you wish to play agai? Y/N: ").lower()
...
def game():
    ...
    if sum_cards(p_cards) > 21:
        finished = input("You have more then 21,You lose! Do you wish to play agai? Y/N: ").lower()
